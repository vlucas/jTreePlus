 ...
 * Modifications by Vance Lucas http://www.vancelucas.com
...
(function($) {
	$.fn.jTree = function(options) {
		...
		var helperItem = '<li class="jTreePlacement" style="background:'+opts.pBg+';border:'+opts.pBorder+';color:'+opts.pColor+';height:'+opts.pHeight+'"></li>';
		var container = this;
		//events are written here
		$(this).find("li").mousedown(function(e){
			if ($("#jTreeHelper").is(":not(:animated)") && e.button !=2) {
				// -------------------------------------------
				// BEGIN MODS for allowing clicks on certain elements - Vance Lucas
				if (!e) var e = window.event;
				var targ;
				if (e.target) targ = e.target; else if (e.srcElement) targ = e.srcElement;
				if (targ.nodeType == 3) targ = targ.parentNode; // defeat Safari bug
				var matchedElement = false;
				// Elements to allow clicks for
				var ignoreElements = ['A', 'FORM'];
				$.each(ignoreElements, function(key, val) {
					// Ensure parent elements are not one of the ignored elements
					$(targ).parents().andSelf().each(function() {
						if(this.tagName.toLowerCase() == val.toLowerCase()) {
							matchedElement = true; // break out of loop, found match in parents
							return;
						}
					});
					if(matchedElement === true) { return; }
				});
				if(matchedElement === true) { return false; }
				// END MODS
				// -------------------------------------------
				
				//$("body").css("cursor","move");
				// append jTreePlacement to body and hides
				$("body").append(helperItem);
		...
		$(this).find("li").andSelf().mouseup(function(e){
			...
			if ($(".jTreePlacement").css('height', $(this).height()).is(":visible")) {
				$(cur).insertBefore(".jTreePlacement").fadeIn('fast');
			}
			$(cur).show();
			...
			//inserting after
				...
				// as a child
					...
					if ($(this).find("ul").length == 0)
						$(this).append('<ul>'+helperItem+'</ul>');
					else
						$(this).find("ul").prepend(helperItem);
				}
			}
